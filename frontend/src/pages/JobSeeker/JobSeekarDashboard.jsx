...
const JobSeekarDashboard = () => {
  ...
  useEffect(() => {
    const timeoutId = setTimeout(() => {
      ...
      //Only cal API if there are meaningful filters
  ...
  //       toast.success("Job saved successfull!");
  //     }
  //     fetchJobs();
  //   } catch (error) {
...
const toggleSaveJob = async (jobId, isSaved) => {
  try {
    // ✅ Optimistically update UI first (use _id instead of id)
    setJobs((prevJobs) =>
      prevJobs.map((job) =>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 lg:py-8">
          ...
          <div className="flex gap-6 lg:gap-8">
            ...
            <div className="hidden lg:block w-80 flex-shrink-0">
              <div className="bg-white/80 backdrop-blur-xl rounded-2xl shadow-2xl border border-white/20 p-6 sticky top-20">
                ...
                className={
                  ...
                  {jobs.map((job) => (
                    ...
                    onApply={() => applyToJob(job._id)}
                      onToggleSave={() => toggleSaveJob(job._id, job.isSaved)}   // <-- add this
  saved={job.isSaved}   
                    />
                  ))}
                </div>
...
export default JobSeekarDashboard

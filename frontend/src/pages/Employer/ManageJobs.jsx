...
const ManageJobs = () => {
  ...
  const handleDeleteJob = async (jobID) => {
    try {
      ...
      toast.success("Job listing deleted sucessfully")
    } catch (error) {
    ...
    try {
      ...
      if (response.status === 200 && response.data?.length > 0) {
        const formattedJobs = response.data?.map((job) => ({
          ...
          datePosted: moment(job?.createdAt).format("DD-MM-YYY"),
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              ...
              <div className="sm:w-48">
                <select
                  ...
                  className='block w-full px-4 py-2 text-sm border border-gray-200 rounded-lg focus:ring-blue-500/20 focus:border-blue-500 transition-all duration-200 '>
                        ...
                        ? Array.from({ length: 5 }).map((_, index) => (
                          ...
                          <tr
                            ...
                            <td className="px-6 py-4 whitespace-nowrap text-sm font-medium min-w-[180px] sm:min-w-0">
                              <div className="flex space-x-2">
                                <button
                                  className="text-blue-600 hover:text-blue-800 p-2 rounde-lg hover:bg-blue-50 transition-colors duration-200"
                                  onClick={() =>
                                    navigate("/post-job", {
                                      state: { jobId: job.id },
                                    })
